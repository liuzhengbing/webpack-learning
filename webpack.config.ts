...
// css摇树会将antd.css清除，因此这里采用antd的按需加载，不使用css摇树；且css摇树无法处理动态添加class这种情况，建议不用
// import PurgecssPlugin from 'purgecss-webpack-plugin'; // css摇树
// import glob from 'glob';

// 打包分析工具
import ProgressBarPlugin from 'progress-bar-webpack-plugin'; // 打包进度条
import SpeedMeasurePlugin from 'speed-measure-webpack-plugin'; // 构建耗时分析
...
const config: webpack.Configuration = {
  ...
  plugins: [
    // 对于index.html的浏览器缓存设置是Cache-Control: no-store, no-cache, must-revalidate，不会进行缓存
...
// smp 和 MiniCssExtractPlugin有冲突
const exportConfig = isProduction ? config : smp.wrap(config as any);
